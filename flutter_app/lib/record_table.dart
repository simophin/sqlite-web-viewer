...
class RecordTable<CellType> extends HookWidget {
  ...
  final int rowCount;
  final (String, TextStyle?) Function(BuildContext context, int rowIndex, int columnIndex)
  cellValue;
    ...
    final measures = useMemoized(
      () => columnSampleTexts.fold(
        ...
        (m, colTexts) {
          ...
          final (maxWidth, maxHeight) = colTexts.foldIndexed((_minColumnWidth, 0.0), (columnIndex, acc, text) {
            final isPrimaryKeyColumn = primaryKeyColumns.contains(columns[columnIndex]);
              ...
              return (
                max(
                  ...
                  min(_maxColumnWidth, painter.maxIntrinsicWidth) +
                      ...
                      (isPrimaryKeyColumn ? (_primaryKeyIconGap + _primaryKeyIconSize) : 0.0) + // Extra space for primary key icon
                      2.0,
                ), // +2 for the border
                max(
                  acc.$2,
